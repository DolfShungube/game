...
export class Level1 {
  ...
  async init() {
    ...
    this.bookshelf = this.Bookshelf.loadBookshelf(
      this.scene,
      { x: -12, y: 0, z: 20 },
      6.2,
      Math.PI / 2
    ...
    // Temporary ambient light for initial room visibility - will fade out
    this.ambientLight = new THREE.AmbientLight(0xffffff, 0.9);

    // Main ceiling light from the living_lamp model - starts off, will fade in
    this.ceilingPointLight = new THREE.PointLight(0xfff8e1, 0, 50, 1.8);
    this.ceilingPointLight.position.set(0, 13, 0);
    this.ceilingPointLight.castShadow = true;
    this.ceilingPointLight.shadow.mapSize.width = 1024;
    this.ceilingPointLight.shadow.mapSize.height = 1024;

    // Ceiling spotlight from lamp - starts off, will fade in
    ...
    this.ceilingSpotlight.position.set(0, 14.5, 0);
    this.ceilingSpotlight.target.position.set(0, 0, 0);
    this.ceilingSpotlight.castShadow = true;
    this.ceilingSpotlight.shadow.mapSize.width = 2048;
    this.ceilingSpotlight.shadow.mapSize.height = 2048;
    // light for the table lamp
    this.Tablelamp = new THREE.SpotLight(
      0xfff4e6,
      0,
      45,
      Math.PI / 4,
    ...
    this.Tablelamp.position.set(-20, 0.5, -10.0);
    this.Tablelamp.target.position.set(0, 0, 0);
    this.Tablelamp.castShadow = true;
    this.Tablelamp.shadow.mapSize.width = 2048;
    this.Tablelamp.shadow.mapSize.height = 2048;
    // Bookshelf accent spotlight1 - starts off, will fade in
    this.bookshelfSpotlight1 = new THREE.SpotLight(
      0xffffff,
      0,
      30,
      Math.PI / 4,
    ...
    this.bookshelfSpotlight2 = new THREE.SpotLight(
      0xffffff,
      0,
      25,
      Math.PI / 3,
    ...
    // Wall accent lights - starts off, will fade in
    this.wallLight1 = new THREE.PointLight(0xffaa66, 0, 15, 2);
    this.wallLight1.position.set(0, 8, 20);

    this.wallLight2 = new THREE.PointLight(0xffaa66, 0, 15, 2);
    this.wallLight2.position.set(0, 8, -20);

    this.wallLight3 = new THREE.PointLight(0xffaa66, 0, 15, 2);
    this.wallLight3.position.set(-20, 8, 0);

    this.wallLight4 = new THREE.PointLight(0xffaa66, 0, 15, 2);
    this.wallLight4.position.set(20, 8, 0);
    ...
    // Add all lights to the room
    this.room.add(this.ambientLight);
    this.room.add(this.ceilingPointLight);
    ...
    this.room.add(this.wallLight1);
    this.room.add(this.wallLight2);
    this.room.add(this.wallLight3);
    this.room.add(this.wallLight4);
    this.room.add(this.Tablelamp);
  ...
  startGame() {
    ...
    // Transition from ambient to ceiling light
  ...
  transitionLighting() {
    // Smoothly transition from ambient light to ceiling and accent lights
    const duration = 2500; // 2.5 seconds
    const startTime = Date.now();
    const initialAmbient = 0.4;
    const finalAmbient = 4.5;

    // Final light intensities
    const finalCeilingPoint = 50.0;
    const finalCeilingSpot = 50.0;
    const finalBookshelfSpot1 = 15.5;
    const finalBookshelfSpot2 = 15.0;
    const finalWallLight = 0;
    ...
    const animate = () => {
      ...
      // Fade out ambient light
      this.ambientLight.intensity =
        initialAmbient - (initialAmbient - finalAmbient) * eased;

      // Fade in main ceiling lights
      this.ceilingPointLight.intensity = finalCeilingPoint * eased;
      this.ceilingSpotlight.intensity = finalCeilingSpot * eased;

      // Fade in bookshelf spotlights
      this.bookshelfSpotlight1.intensity = finalBookshelfSpot1 * eased;
      this.bookshelfSpotlight2.intensity = finalBookshelfSpot2 * eased;

      // Fade in wall accent lights
      this.wallLight1.intensity = finalWallLight * eased;
      this.wallLight2.intensity = finalWallLight * eased;
      this.wallLight3.intensity = finalWallLight * eased;
      this.wallLight4.intensity = finalWallLight * eased;
